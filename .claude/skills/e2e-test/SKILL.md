...
<!-- Source: coleam00/ai-coding-summit-workshop-2 .claude/skills/e2e-test/SKILL.md — adopted 2026-07-10, near-verbatim (subagent-tool naming modernized; report/screenshot paths stay at project root as a deliberate exception to the .agents/ artifact rule). External dependency: agent-browser CLI (Vercel Labs, npm i -g agent-browser); the companion agent-browser skill is its CLI reference. -->
...
Launch **three sub-agents simultaneously** (the Agent tool; not to be confused with the TaskCreate/TaskUpdate tracking used later). All three run in parallel.
