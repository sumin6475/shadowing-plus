...
Read the project AGENTS.md's **Deployment** section (per-project tweaks live there: platform choice, env promotion, migration commands, edge cases). If absent, apply the defaults:
...
First deploy for this project: link it (`vercel link` / `railway link`), then record the chosen setup in AGENTS.md's Deployment section so next time isn't a first time.
...
- [ ] DB migrations: identified, ordered, with a rollback path (run per AGENTS.md's migration commands — Railway: before or during release per project convention)
