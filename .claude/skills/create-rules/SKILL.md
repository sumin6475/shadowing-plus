---
name: create-rules
description: Generate or refresh the project's CLAUDE.md by analyzing the codebase (brownfield) or deriving from the PRD (greenfield). Run at project setup, or when conventions have drifted from what CLAUDE.md says.
argument-hint: [path to PRD, optional]
disable-model-invocation: true
---

<!-- Source: coleam00/ai-transformation-workshop .claude/commands/create-rules.md; layered CLAUDE.md / CODEBASE_MAP.md / .claudeignore patterns from coleam00/helpline — adapted 2026-07-10 -->

# Create Global Rules

**Input**: $ARGUMENTS

Generate the project's `CLAUDE.md` from evidence, not assumption: real code for brownfield, the PRD's decisions for greenfield.
...
- **Refresh** — `CLAUDE.md` already exists → edit it in place against current evidence; don't regenerate from the template (the template is only for first generation and lives upstream in Build System).
- **Brownfield** — substantial code exists, no CLAUDE.md → extract the conventions that already exist in the code. Do not invent new ones.
...
Find the **real, runnable** lint, typecheck, test, and build commands (from package scripts, pyproject, CI config, or README). These go into CLAUDE.md's Validation section as the baseline command set: `/validate` runs them plus e2e/smoke as the full gauntlet, and they must be kept in sync with `.claude/hooks/hooks_config.json` (the Stop hook enforces lint+test mechanically from there). Verify each command actually runs before writing it down. Then replace `hooks_config.json`'s `"auto"` entries with the explicit verified commands (see its `_explicit_example`), so the gate stops depending on detection heuristics.
...
First generation: fill `CLAUDE-template.md` (project root) into **`CLAUDE.md`** (project root). Refresh: edit the existing `CLAUDE.md` in place, preserving its structure.
...
**File**: `CLAUDE.md` {+ per-area files / CODEBASE_MAP.md / .claudeignore if generated}
...
Then ask the user to review `CLAUDE.md` before treating setup as done. After approval of a first generation, delete `CLAUDE-template.md` from the project — the template lives upstream in Build System, and later `/create-rules` runs refresh `CLAUDE.md` in place without needing it.
...
- Update CLAUDE.md as the project evolves; `/system-review` will propose edits when reality drifts from the rules.
