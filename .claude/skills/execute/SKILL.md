...
<!-- Source: coleam00/habit-tracker .claude/commands/core_piv_loop/execute.md (task execution discipline, output report; its plan-template COMPLETION CHECKLIST is folded into Step 6 here); clean-tree pre-flight and incremental/full validation split from context-engineering-intro use-cases/ai-coding-wisc-framework .claude/commands/execute.md, fresh-session framing from that use-case's README — adapted 2026-07-10 -->
...
**Context reset**: this skill is designed to run in a fresh session, so planning-phase research doesn't pollute implementation context. The plan file and the project's CLAUDE.md are all you need — the plan was written to pass the No Prior Knowledge Test. If something essential is missing from the plan, that is a plan defect: note it for the execution report rather than silently improvising.
...
- Note the current branch; if the project uses feature branches, create one per its CLAUDE.md git-workflow conventions. If none are documented, ask the user rather than guessing.
...
2. **Implement** exactly what the task specifies, consistent with the project's existing patterns and CLAUDE.md conventions (types, docs, logging as the project does them).
