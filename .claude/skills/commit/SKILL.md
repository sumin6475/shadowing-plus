...
<!-- Source: context-engineering-intro use-cases/ai-coding-wisc-framework .claude/commands/commit.md (conventional tags, WHY body, Context section = git log as memory); atomic-commit minimalism from coleam00/habit-tracker .claude/commands/commit.md; PRD-status step is ours (see Build System MEMORY.md decision) — adapted 2026-07-10 -->
...
**Do NOT stage:** `.env` or credential files, large binaries, `e2e-screenshots/` and generated test reports (e2e artifacts stay local; the project's `.gitignore` should cover them), the reflector's transient files (`.claude/claude-md-review.md`, `.claude/.claude-md-review-state` — drafts get adjudicated by /system-review, not committed), files unrelated to the current task.
...
- `CLAUDE.md` (root or per-area) — rules changes
- `.claude/skills/` — skills created or modified
- `.claude/agents/` — subagent definitions
- `.claude/hooks/` or `.claude/settings.json` — hook changes
...
Context:
- Updated CLAUDE.md error-handling rules with the retry convention
...
- **No push** — pushing/PR creation only when the user asks or the project's CLAUDE.md git workflow says so
